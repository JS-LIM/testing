...
****************************************************************************/
{
   ...
   public void Cmd_Q(File_Buffer FILE, boolean b){
	   b = true;
	   FILE.setUpdateFlag(UPDATE_FILE);
   }
   
   public void Cmd_X(File_Buffer FILE, boolean b){
	   b = true;
	   FILE.setUpdateFlag(!UPDATE_FILE);
   }
   ...
   public void Cmd_D(File_Buffer FILE, int nLines)
   {
       ...
       if(Valid_Lines(FILE,nLines)) 
       {
          ...
          for(i=CLN; i<=end; i++) //결함을 수정하기 위해 i<end를 i<=end로 수정하였다.
              FILE.DelLine(CLN);
   ...
   public void Cmd_K(String keyword)
   {
       ...
       for(i=1; i<=num_kw; i++) 
       {
           ...
           // 문자열 keyword의 존재를 확인하기 위해 조건문 if 수정
           if((kwRec.getKW()).contains(keyword) == true) 
           {
